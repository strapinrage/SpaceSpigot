...
public class PingCommand extends Command {
	...
	public PingCommand(String name) {
		...
		this.setPermission("spacespigot.command.ping");
	}
	...
	public boolean execute(CommandSender sender, String currentAlias, String[] args) {
		...
		if ((args.length == 0) && sender instanceof Player) {
			String finalString = ChatColor.translateAlternateColorCodes('&', SpaceSpigotConfig.pingSelfCmdString)
			...
			if (pingPlayer != null && Bukkit.getOnlinePlayers().contains(pingPlayer)) {
				String finalString = ChatColor.translateAlternateColorCodes('&', SpaceSpigotConfig.pingOtherCmdString)
