package net.strapinrage.spacespigot.config;
...
import net.strapinrage.spacespigot.async.pathsearch.AsyncNavigation;
import net.strapinrage.spacespigot.entity.EntityTickLimiter;

@SuppressWarnings("unused")
public class SpaceSpigotConfig {

	private static final Logger LOGGER = LogManager.getLogger(SpaceSpigotConfig.class);
	public static File CONFIG_FILE;
	protected static final YamlCommenter c = new YamlCommenter();
	private static final String HEADER = "SpaceSpigot - Custom server software developed by strapinrage for spacecraft network.\n"
			+ "Official Discord: https://dc.spacecraft.net/\n"
			+ "Report issues: https://github.com/strapinrage/SpaceSpigot/issues\n";
	...
	public static void init(File configFile) {
		...
		try {
			SpaceSpigot.LOGGER.info("Loading SpaceSpigot config from " + configFile.getName());
			config.load(CONFIG_FILE);
		} catch (IOException ignored) {
		} catch (InvalidConfigurationException ex) {
			LOGGER.log(Level.ERROR, "Could not load spacespigot.yml, please correct your syntax errors", ex);
		...
		c.setHeader(HEADER);
		readConfig(SpaceSpigotConfig.class, null);

		// Move nacho config options to our config
		/*if (!NachoConfig.hasMigrated) {
			// TODO: finish config migration/loading
			//NachoConfig.moveToSpaceSpigotConfig(NachoConfig.class, SpaceSpigotConfig.class);
			LOGGER.info("Successfully loaded nacho.yml into memory!");
			LOGGER.warn("Loading of nacho.yml will be removed in the future, so transfer the config options into spacespigot.yml, then delete nacho.yml");
			makeReadable();
		}*/
		...
		try {
			...
			LOGGER.warn("Please regenerate your spacespigot.yml file to prevent this issue! The server will run with the default config for now.");
	...
	static void loadComments() {
		...
		c.addComment("settings.command", "Configuration for SpaceSpigot's commands");
		...
		c.addComment("settings.command.mob-ai", "Enables the command \"/mobai\" which toggles mob ai. Users require the permission spacespigot.command.mobai");
		...
		c.addComment("settings.command.ping.enable", "Enables the command \"/ping <player>\" which shows player ping. Users require the permission spacespigot.command.ping");
		...
		c.addComment("settings.statistics", "Enables SpaceSpigot statistics. This allows developers to see how many SpaceSpigot servers are running. \nThis has no performance impact and is completely anonymous, but you can opt out of this if you want.");
	...
	private static void serverBrandName() {
		serverBrandName = getString("settings.brand-name", "SpaceSpigot");
	}
