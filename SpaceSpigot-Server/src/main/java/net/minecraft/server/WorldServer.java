...
import net.strapinrage.spacespigot.async.entitytracker.MultithreadedEntityTracker;
import net.strapinrage.spacespigot.config.SpaceSpigotConfig;
import net.strapinrage.spacespigot.world.WorldTicker;
...
public class WorldServer extends World implements IAsyncTaskHandler {
	...
	public WorldServer(MinecraftServer minecraftserver, IDataManager idatamanager, WorldData worlddata, int i,
		...
		// SpaceSpigot - async entity tracking
		if (SpaceSpigotConfig.disableTracking) {
			this.tracker = new EntityTracker(this);
		} else if (SpaceSpigotConfig.fullAsyncTracking) {
	...
	public void doTick() {
		...
		if (this.getGameRules().getBoolean("doMobSpawning")
			...
			// SpaceSpigot start - disable mob spawning based on tps and server load
			if (!(SpaceSpigotConfig.limitedMobSpawns
					&& MinecraftServer.getServer().recentTps[0] < SpaceSpigotConfig.limitedMobSpawnsThreshold) // check if tps is low
					&& !(SpaceSpigotConfig.stopMobSpawnsDuringOverload && lastTickOverload)) { // check if server is overloaded
				// SpaceSpigot end
