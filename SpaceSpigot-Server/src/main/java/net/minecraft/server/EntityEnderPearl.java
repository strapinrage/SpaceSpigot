...
public class EntityEnderPearl extends EntityProjectile {
	...
	protected void a(MovingObjectPosition movingobjectposition) {
		...
		if (blockPosition != null) {
			...
			boolean collides = SpaceSpigotConfig.pearlPassthroughTripwire && block == Blocks.TRIPWIRE
					|| SpaceSpigotConfig.pearlPassthroughCobweb && block == Blocks.WEB
					|| SpaceSpigotConfig.pearlPassthroughBed && block == Blocks.BED
					|| SpaceSpigotConfig.pearlPassthroughFenceGate
					...
					|| SpaceSpigotConfig.pearlPassthroughSlab && (block == Blocks.STONE_SLAB
		...
		if (!this.world.isClientSide) {
			if (entityliving instanceof EntityPlayer) {
				...
				if (entityplayer.playerConnection.a().isConnected() && entityplayer.world == this.world
					...
					if (SpaceSpigotConfig.antiEnderPearlGlitch) {
					...
					if (!teleEvent.isCancelled() && !entityplayer.playerConnection.isDisconnected()) {
						...
						SpaceSpigot.getInstance().getLagCompensator().registerMovement(player, location); // Nacho
