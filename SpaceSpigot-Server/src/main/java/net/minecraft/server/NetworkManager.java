...
import net.strapinrage.spacespigot.exception.ExploitException;
	...
	protected void a(ChannelHandlerContext ctx, Packet packet) throws Exception {
		if (this.channel.isOpen()) {
			if (this.m instanceof PlayerConnection) {
				try {
					for (net.strapinrage.spacespigot.protocol.PacketListener packetListener : SpaceSpigot.getInstance()
							.getPacketListeners()) {
	...
	public void handle(Packet packet) {
		if (this.isConnected()) {
			...
			if (!shouldCheckPacket) {
		        ...
		        if (SpaceSpigotConfig.asyncKnockback && (packet instanceof PacketPlayOutEntityVelocity || packet instanceof PacketPlayOutPosition || packet instanceof PacketPlayInFlying.PacketPlayInPosition || packet instanceof PacketPlayInFlying)) {
		        	// Send it with high priority
		        	SpaceSpigot.getInstance().getKnockbackThread().addPacket(packet, this, null);
		            return;
		        }
			}
	        // SpaceSpigot end
