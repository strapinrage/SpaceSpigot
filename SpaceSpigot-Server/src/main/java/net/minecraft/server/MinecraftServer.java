...
import net.strapinrage.spacespigot.tickloop.ReentrantIAsyncHandler;
import net.strapinrage.spacespigot.tickloop.TasksPerTick;
import net.strapinrage.spacespigot.world.WorldTickManager;
// SpaceSpigot end
...
public abstract class MinecraftServer extends ReentrantIAsyncHandler<TasksPerTick> implements ICommandListener, IAsyncTaskHandler, IMojangStatistics {
	...
	public java.util.Queue<Runnable> priorityProcessQueue = new java.util.concurrent.ConcurrentLinkedQueue<Runnable>(); // SpaceSpigot
	public int autosavePeriod;
	// CraftBukkit end

	// SpaceSpigot - instance
	protected SpaceSpigot spaceSpigot;
	
	// SpaceSpigot - MSPT for tps command
	private double lastMspt;

	// SpaceSpigot start - backport modern tick loop
	...
	public MinecraftServer(OptionSet options, Proxy proxy, File file1, Thread thread) {
		super("Server"); // SpaceSpigot - backport modern tick loop
	...
	public void run() {
		...
		try {
			...
			if (this.init()) {
				//SpaceSpigot - statistics
				disableStatistics = true;
				// SpaceSpigot start - implement thread affinity
				if (SpaceSpigotConfig.threadAffinity) {
					LOGGER.info(" ");
					...
					SpaceSpigot.debug(AffinityLock.dumpLocks());

				}
				// SpaceSpigot end

				// SpaceSpigot - parallel worlds
				...
				while (this.isRunning) {
                    ...
                    if (i > 5000L && this.nextTickTime - this.lastOverloadWarning >= 30000L && ticks > 500) { // CraftBukkit // SpaceSpigot - prevent display of overload on first 500 ticks
                        long j = i / 50L;
			...
			// SpaceSpigot start - thread affinity
			if (lock != null) {
				lock.release();
				MinecraftServer.LOGGER.info("Released CPU " + lock.cpuId() + " from server usage.");
			}
			// SpaceSpigot end
			// SpaceSpigot start - stop statistics connection
			Thread statisticsThread = null;
			if (disableStatistics) {
				StatisticsClient client = this.getSpaceSpigot().getClient();
			...
			// SpaceSpigot - wait for statistics to finish stopping
			try {
				if (this.getSpaceSpigot().getClient().isConnected) {
...
//                }
	...
	// SpaceSpigot - backport modern tick loop
...
//        this.methodProfiler.b();
	...
	public void B() {
		...
		// SpaceSpigot - move to WorldTickManager
...
//		SpigotTimings.bukkitSchedulerTimer.stopTiming(); // Spigot
		...
		// SpaceSpigot start - priority process queue
	...
	public String getServerModName() {
		return SpaceSpigotConfig.serverBrandName; // [Nacho-0035] // NachoSpigot - NachoSpigot > // TacoSpigot - TacoSpigot //
	...
	// SpaceSpigot start - backport modern tick loo
	...
	// SpaceSpigot - instance
	public SpaceSpigot getSpaceSpigot() {
		return this.spaceSpigot;
	}
	
	// SpaceSpigot - MSPT (milliseconds per tick)
