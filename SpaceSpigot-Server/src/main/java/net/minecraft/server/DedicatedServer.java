...
import net.strapinrage.spacespigot.commons.IPUtils;
...
public class DedicatedServer extends MinecraftServer implements IMinecraftServer {
    ...
    public DedicatedServer(joptsimple.OptionSet options, Thread thread1) { // SpaceSpigot - backport modern tick loop
        super(options, Proxy.NO_PROXY, DedicatedServer.a, thread1);
		// CraftBukkit end
		if (!SpaceSpigotConfig.disableInfiniSleeperThreadUsage) {
	...
	protected boolean init() throws IOException {
		...
		if (!this.p.a() && !eulaAgreed) { // Spigot
			...
			SpaceSpigotConfig.init((File) options.valueOf("spacespigot-settings"));
			// SpaceSpigot end
			...
			if (!this.getOnlineMode()) {
				...
				if (org.spigotmc.SpigotConfig.bungee) {
					...
					if (!SpaceSpigotConfig.stopNotifyBungee) {
						DedicatedServer.LOGGER
								.warn("---------------------------- SpaceSpigot Checker ----------------------------");
						DedicatedServer.LOGGER.warn(
								"If you don't want to see this message anymore, set \"settings.stop-notify-bungee\" to \"true\" in \"spacespigot.yml\"!");
			...
			if (!NameReferencingFileConverter.a(this.propertyManager)) {
				...
				this.spaceSpigot = new SpaceSpigot(); // SpaceSpigot
