...
import net.strapinrage.spacespigot.events.PlayerIllegalBehaviourEvent;
...
public class PlayerConnection implements PacketListenerPlayIn, IUpdatePlayerListBox {
	...
	// SpaceSpigot - queue-able packets
	...
	public void c() {
		...
		if (SpaceSpigotConfig.modernKeepalive) {
		...
		if (this.b(packetplayinflying)) {
			...
			if (!this.player.viewingCredits) {
				...
				if ((packetplayinflying.hasPos) && ((delta > 0.0D) && (this.checkMovement && !this.player.dead))) {
					for (MovementListener movementListener : SpaceSpigot.getInstance().getMovementListeners()) {
						try {
				...
				if ((delta > 1f / 256 || deltaAngle > 10f) && (this.checkMovement && !this.player.dead)) {
					...
					if (SpaceSpigotConfig.firePlayerMoveEvent && PlayerMoveEvent.getHandlerList().getRegisteredListeners().length != 0) { // SpaceSpigot - only handle player move event when there are listeners // Spigot - don't skip any move events
						...
						if (event.isCancelled()) {
		                    SpaceSpigot.getInstance().getLagCompensator().registerMovement(player, to); // Nacho
                    ...
                    SpaceSpigot.getInstance().getLagCompensator().registerMovement(player, to); // Nacho - register movement
				}
	...
	public void a(PacketPlayInBlockPlace packetplayinblockplace) {
		...
		if (packetplayinblockplace.getFace() == 255) {
			...
			if (!throttled) {
				...
				// SportPaper end
			} else if (MinecraftServer.currentTick - lastDropTick > 1 && SpaceSpigotConfig.fixEatWhileRunning) {
				this.player.playerInteractManager.useItem(this.player, this.player.world, itemstack);
			}
		...
		if (itemstack == null || itemstack.l() == 0) {
			...
			// KigPaper-0172
			if (SpaceSpigotConfig.stopDecodingItemStackOnPlace
	...
	public void sendPacket(final Packet packet) {
		...
		try {
			for (PacketListener packetListener : SpaceSpigot.getInstance().getPacketListeners()) {
				try {
	...
	public void a(PacketPlayInUseEntity packetplayinuseentity) {
		...
		if (entity != null) {
			...
			if (!flag) {
                ...
                d0 = (SpaceSpigotConfig.improvedHitDetection) ? 12.75D : 9.0D;
            } else {
            	// SpaceSpigot start - configurable reach cap
            	if (SpaceSpigotConfig.creativeBypass && this.player.playerInteractManager.getGameMode() == EnumGamemode.CREATIVE) {
            		d0 = 36.0;
            	} else {
                	d0 = SpaceSpigotConfig.maxReachSqrd;
            	}	
            }
...
//        }
		...
		} else if (SpaceSpigotConfig.kickOnIllegalBehavior) {
	...
	// SpaceSpigot start - queue-able packets
