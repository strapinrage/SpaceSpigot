...
public abstract class World implements IBlockAccess {
	...
	public final List<Entity> entityList = new ObjectArrayList<Entity>() { // SpaceSpigot - ArrayList -> ObjectArrayList
		@Override
	...
	public final List<EntityHuman> players = new ObjectArrayList<>(); // SpaceSpigot - ArrayList -> ObjectArrayList
	public final List<Entity> k = new ObjectArrayList<>(); // SpaceSpigot - ArrayList -> ObjectArrayList
	...
	protected boolean lastTickOverload = false; // SpaceSpigot
	...
	protected World(IDataManager idatamanager, WorldData worlddata, WorldProvider worldprovider,
		...
		timings = new co.aikar.timings.WorldTimingsHandler(this); // Spigot - code below can generate new world and
																	// access timings
		// SpaceSpigot - re-implement Spigot's entity max tick time, but only for certain entities
		this.entityLimiter = new EntityTickLimiter(SpaceSpigotConfig.entityMaxTickTime); //new org.spigotmc.TickLimiter(spigotConfig.entityMaxTickTime);
		this.tileLimiter = new org.spigotmc.TickLimiter(SpaceSpigotConfig.tileMaxTickTime);
	}
	...
	public boolean addEntity(Entity entity, SpawnReason spawnReason) { // Changed signature, added SpawnReason
		...
		if (!flag && !this.isChunkLoaded(i, j, true)) {
			...
			// SpaceSpigot start - configurable entity hit delay
			if (entity instanceof EntityLiving) {
				((EntityLiving) entity).maxNoDamageTicks = SpaceSpigotConfig.hitDelay;
			}
			// SpaceSpigot end
			
			// SpaceSpigot start - configurable potion speeds
			if (entity instanceof EntityPotion) {
				// Subtract the speed from the y value of the velocity, the potion will drop faster and will therefore splash faster
				entity.motY -= SpaceSpigotConfig.potionSpeed;
				
				// Mark velocity as changed only if potion speed is not default
				if (!(SpaceSpigotConfig.potionSpeed == 0)) {
...
//        if (!guardEntityList) { // Spigot - It will get removed after the tick if we are ticking // Paper - moved down
	...
	public void tickEntities() {
		boolean tickOverload = false; // SpaceSpigot
		...
		entityLimiter.initTick(); // SpaceSpigot - re-implement Spigot's entity max tick time, but only for certain entities
		for (tickPosition = 0; tickPosition < entityList.size(); tickPosition++) {
			...
			// SpaceSpigot - synchronize
			synchronized (entity) {
				// SpaceSpigot start - re-implement Spigot's entity max tick time, but only for certain entities
...
//            this.tileEntityList.removeAll(this.c);//c  == tileEntityListUnload
		...
		lastTickOverload = tickOverload; // SpaceSpigot
	}
	...
	public float a(Vec3D vec3d, AxisAlignedBB axisalignedbb) {
		...
		// SpaceSpigot start - switch to mathhelper instead of Java math
