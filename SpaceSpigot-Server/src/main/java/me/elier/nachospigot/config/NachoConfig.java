...
public class NachoConfig {
	...
	// If the config has been migrated to spacespigot.yml
	...
	public static void init(File configFile) {
		...
		try {
			SpaceSpigot.LOGGER.info("Loading NachoSpigot config from " + configFile.getName());
	...
	// Migrates all read data into the SpaceSpigot config
	// TODO: save this data to file instead of just putting it in memory
	public static void moveToSpaceSpigotConfig(Class<?> clazz, Class<?> spaceSpigotConfigClazz) {
		for (Field oldField : clazz.getDeclaredFields()) {
			...
			if (Modifier.isPublic(oldField.getModifiers()) && Modifier.isStatic(oldField.getModifiers()) && !oldField.getName().equals("CONFIG_FILE") && !oldField.getName().equals("hasMigrated")) {
				...
				for (Field newField : spaceSpigotConfigClazz.getDeclaredFields()) {
				...
				if (!found) {
					SpaceSpigot.LOGGER.warn("Unable to migrate option " + oldField.getName() + "!");
				}
			}
 		}
		hasMigrated = true;
